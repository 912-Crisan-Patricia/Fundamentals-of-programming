class UI:
    ...
    def create1(self):
        ...
        if guests<1 or guests>4 :
            ...
            ok=self._service.create(name,room_type,guests,ad,am,dd,dm)
            if ok==1:
    ...
    def delete2(self):
        ...
        ok=self._service.find_reservation_id(n)
        if ok==-1:
            print("Error:We couldn't delete a reservation that doesn't exist!")
        else:
            self._service.delete_reservation(n)
    ...
    def available_rooms(self):
        ...
        list=self._service.available_date(int(arrival[0]),int(arrival[1]),int(departure[0]),int(departure[1]))
        for i in list:
            print(str(i))
        if len(list)==0:
    ...
    def monthly_report(self):
        list=self._service.monthly_report()
        for l in list:
            print(str(l[0]),l[1])

    def daily(self):
        message=self._service.days_report()
        print(message)